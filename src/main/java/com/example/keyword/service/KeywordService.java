...
import com.example.keyword.repository.ChildKeywordRepository;
...
public class KeywordService {
    ...
    private final ChildKeywordRepository childKeywordRepository;

    //의미없는 키워드 제외를 위한 목록
    //추후 업데이트
    Set<String> meaninglessWordSet = new HashSet<>(Arrays.asList(
            ...
            "발부", "기자", "행사", "최대", "네이버뉴스", "선정", "사업", "언론사", "저장", "관련", "업체",
            "옵션", "바로가기", "전체", "문서", "선택", "검색", "댓글", "부문", "내일", "접기", "동영상",
            "지식", "메인", "레이어", ""));
    ...
    public void extractKeyword(CrawlingResponseDto crawlingResponseDto) {
        ...
        //의미없는 키워드 제외를 위한 목록
        //추후 업데이트
        Set<String> meaninglessWords = meaninglessWordSet;
    ...
    public void extractChildKeyword(CrawlingResponseDto crawlingResponseDto, String parentKeyword) {
        ...
        Set<String> meaninglessWords = meaninglessWordSet;
        meaninglessWords.add(parentKeyword);
        ...
        createChildKeyword(parentKeyword, topKeywords);
    }

    public void createChildKeyword(String parentKeyword, List<String> topKeywords) {
        ChildKeyword childKeyword = new ChildKeyword().toBuilder()
                .parentKeyword(parentKeyword)
                .childKeywordList(topKeywords)
                .createdTime(LocalDateTime.now())
                .build();

        childKeywordRepository.save(childKeyword);
    }

    public ChildKeyword getRecentChildKeywords(String parentKeyword) {

        try {
            return childKeywordRepository.findFirstByParentKeywordEqualsOrderByCreatedTimeDesc(parentKeyword);
        } catch (Exception e) {
            return null;
        }
    }
